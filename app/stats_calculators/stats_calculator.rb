...
class StatsCalculator
  ...
  def sharpe_ratio(series)
    self.stddev(series).zero? ? 0 : (Math.sqrt(250) * (self.mean(series) / self.stddev(series))).round(2)
  end

  def sortino_ratio(series)
    mean = self.mean(series)
    dr = downside_risk(series, mean)
    dr.zero? ? 0 : (Math.sqrt(250) * (mean / dr)).round(2)
  end
  ...
  def beta(series, market_indexes)
    ...
    (Statsample::Bivariate.covariance(index_daily_return_values, series_daily_return_values) / index_daily_return_values.variance).round(2)
 	end

  def performance_in_days(series, days)
    ((((1 + series.sum/100)**(days.to_f/series.days_elapsed.to_f))-1)*100).round(2)
  end
  ...
  ['sum', 'mean', 'max', 'min'].each do |summary_method|
    define_method(summary_method) do |series|
      series.daily_return_values.send(summary_method).round(2)
    end
  end
end